...
const getGalaxiesDistance = (
  ...
  const rowEmptyCount = Math.abs(
    countElementLessThen(emptyRows, galaxyA[0]) -
      countElementLessThen(emptyRows, galaxyB[0]),
  );
  const colEmptyCount = Math.abs(
    countElementLessThen(emptyCols, galaxyA[1]) -
      countElementLessThen(emptyCols, galaxyB[1]),
  );
...
const getTotalDistance = (
  ...
  for (let i = 0; i < galaxies.length; i++) {
    for (let j = i + 1; j < galaxies.length; j++) {
      totalDistance += getGalaxiesDistance(
        galaxies[i],
        galaxies[j],
        factor,
        emptyRows,
...
const part1 = (rawInput: string) => {
  const { galaxies, emptyColumns, emptyRows } = parseInput(rawInput);
