...
const parseInput = (rawInput: string) => {
  const seeds = rawInput
    .slice(7, rawInput.indexOf("\n\n"))
    .split(" ")
    .map(Number);
  const maps = rawInput
    .trim()
    .slice(rawInput.indexOf("\n\n") + 2)
    .split("\n\n")
    .map((block) =>
      block
        .split("\n")
...
const splitRangesByMap = ([start, rangeLength]: number[], map: number[][]) => {
  ...
  while (index < rangeEnd) {
    let mapping = map.find(
      ([, source, length]) => source <= index && source + length > index,
    );
    ...
    if (mapping) {
      ...
      const [, cEnd] = map.find(
        ([, source]) => source > index && source < rangeEnd,
      ) ?? [0, rangeEnd];
...
const part2 = (rawInput: string) => {
  ...
  const seedRanges = Array.from({ length: seeds.length / 2 }, (_, index) =>
    seeds.slice(index * 2, index * 2 + 2),
  );

  const locationRanges = maps.reduce(
    (sourceRanges, map) =>
      sourceRanges.flatMap((range) => splitRangesByMap(range, map)),
    seedRanges,
  );
